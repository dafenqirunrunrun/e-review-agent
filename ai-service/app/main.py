...
from app.api.agent_rag_internal import router as agent_rag_internal_router
from app.api.agent_framework import router as agent_framework_router
from app.api.agent_rag import router as agent_rag_router
...
from app.agent_trace.runtime_context import AgentTraceContextMiddleware
from app.core.config import settings
from app.policy_rag.embedding import warmup_policy_embedding_provider
from app.policy_rag.reranker import warmup_policy_reranker
...
app.add_middleware(AgentTraceContextMiddleware)
...
app.include_router(agent_rag_router, prefix=settings.api_prefix)
app.include_router(llm_router, prefix=settings.api_prefix)
app.include_router(policy_playground_router, prefix=settings.api_prefix)
...
app.include_router(agent_rag_internal_router, prefix=settings.api_prefix)


@app.on_event("startup")
def startup_warmup():
    app.state.policy_embedding_warmup = warmup_policy_embedding_provider()
    app.state.policy_reranker_warmup = warmup_policy_reranker()
