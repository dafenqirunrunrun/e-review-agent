...
## v1.0.4 Must-Capture Additions

- Agent Trace state snapshot panel: proves each run has replayable state.
- Agent Trace role summary and step timeline: proves multi-role Agent expression.
- Agent Trace replay comparison: proves original and replay runs are preserved and comparable.
- Agent Eval service health cards: proves local demo dependency visibility.
- Agent Eval failure groups: proves diagnostic explainability.
- Agent Eval quality cards: proves 30-sample evaluation and RAG/HITL metrics.
- Case Knowledge retrieve result: proves local Agentic RAG evidence retrieval without claiming vector database integration.
