...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE litemall_ai_agent_run ADD COLUMN replay_from_run_id int DEFAULT NULL',
    'SELECT 1'
  )
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE litemall_ai_agent_run ADD COLUMN replay_count int DEFAULT 0',
    'SELECT 1'
  )
  FROM information_schema.columns
  WHERE table_schema = @schema_name
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE litemall_ai_agent_run ADD COLUMN is_replay tinyint(1) DEFAULT 0',
    'SELECT 1'
  )
  FROM information_schema.columns
  WHERE table_schema = @schema_name
