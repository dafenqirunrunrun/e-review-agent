  ...
  <div class="item_detail">
    ...
    <van-cell-group class="item_cell_group" v-if="goods">
      <van-cell class="item_info">
        <div>
          <span class="item_price">{{ priceCents(goods.info.retailPrice) | yuan }}</span>
          <span class="item_market_price">{{ priceCents(goods.info.counterPrice) | yuan }}</span>
        </div>
        <div class="item-title">
          {{ goods.info.name }}
        </div>
        <div class="item_intro">{{goods.info.brief}}</div>
        <div class="ai_demo_tip">
          可用于评价治理演示：购买、演示支付、确认收货并发布评价后，后台智能体会自动巡检分析。
        </div>
      </van-cell>
    </van-cell-group>
