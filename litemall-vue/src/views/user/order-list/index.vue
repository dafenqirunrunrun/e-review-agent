      ...
      <van-tab v-for="(tabTitle, index) in tabTitles"
        ...
        <van-list v-model="loading"
          ...
          <van-panel v-for="(el, i) in orderList"
            ...
            <div class="total">合计: {{priceCents(el.actualPrice) | yuan}}（含运费{{priceCentsFromFen(el.post_fee) | yuan}}）</div>
            ...
            <div slot="footer"
              ...
              <van-button size="small"
                          v-if="demoShippingAvailable && el.orderStatus === 201"
                          type="primary"
                          @click.stop="mockShipOrder(el.id)">演示发货</van-button>
              <van-button size="small"
...
<script>
import { aiDemoStatus, orderList, orderDelete, orderConfirm, orderCancel, orderRefund, orderMockShip } from '@/api/api';
...
export default {
  ...
  data() {
    return {
      ...
      demoStatus: {
        ...
        demoShippingEnabled: true
      }
    };
  },
  computed: {
    demoShippingAvailable() {
      return this.demoStatus.demoModeEnabled !== false && this.demoStatus.demoShippingEnabled !== false;
    }
  },
  ...
  methods: {
    ...
    mockShipOrder(id) {
      this.$dialog
        .confirm({ message: '答辩演示模式：确认将该订单推进为已发货状态吗？本操作不调用真实物流。' })
        .then(() => {
          orderMockShip({ orderId: id }).then(() => {
            this.init();
            this.$toast('演示发货成功');
          });
        })
    ...
    commentOrder(id) {
      const order = _.find(this.orderList, item => item.id === id);
      if (!order || !order.goodsList || !order.goodsList.length) {
      ...
      this.$router.push({
        ...
        query: {
          orderId: id,
          orderGoodsId: order.goodsList[0].id
        }
      });
    },
    priceCents(value) {
