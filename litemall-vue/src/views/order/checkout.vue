...
<van-popup v-model="showList" position="bottom">
  ...
  />
    ...
    <van-cell-group>
      <van-cell title="商品金额">
        <span class="red">{{ priceCents(goodsTotalPrice) | yuan}}</span>
      </van-cell>
      <van-cell title="邮费">
        <span class="red">{{ priceCents(freightPrice) | yuan}}</span>
      </van-cell>
      <van-cell title="优惠券">
        <span class="red">-{{ priceCents(couponPrice) | yuan}}</span>
      </van-cell>
    ...
    <div class="demo_order_hint">
      提交订单后可进入演示支付；演示模式不会调用真实支付、物流或退款服务。
    </div>

    <van-submit-bar
      :price="priceCents(actualPrice)"
      label="总计："
      buttonText="提交订单"
      :disabled="isDisabled"
      @submit="onSubmit"
